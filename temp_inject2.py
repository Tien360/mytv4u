...
# using the same modal_code as before
modal_code = """
  ...
  Future<List<dynamic>> _fetchAirSchedule() async {
    try {
      ...
      final originalName = _movie?.originalName;
      final year = _movie?.year != null ? int.tryParse(_movie!.year!) : null;

      if (imdbId == null && originalName == null) return [];

      final seriesId = await TmdbApi.getTvSeriesId(
        imdbId: imdbId,
      ...
      final seriesDetails = await TmdbApi.getTvSeriesDetails(seriesId);
      if (seriesDetails == null) return [];

      final currentSeason = seriesDetails['seasons']?.lastWhere(
        (s) => s['season_number'] > 0,
        orElse: () => null,
      );
      if (currentSeason == null) return [];

      return await TmdbApi.getSeasonEpisodes(seriesId, currentSeason['season_number']);
    } catch (e) {
...
c = re.sub(r"(\s+@override\s+Widget build\(BuildContext context\) \{\s+if \(_isLoading\))", r"\n" + modal_code + r"\1", c, count=1)
