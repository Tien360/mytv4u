...
def fix_all(filepath):
    ...
    match2 = re.search(old_child_text, content, flags=re.MULTILINE)
    if match2:
        content = content.replace(match2.group(0), new_child)
    else:
        print("Could not find old child Text")
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

fix_all('lib/screens/player_screen.dart')
print("Fixed everything using replace")
