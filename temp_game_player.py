...
print("Created game_player_screen.dart")

with open('lib/screens/game_detail_screen.dart', 'r', encoding='utf-8') as f:
    g_c = f.read()

target_import = "import '../widgets/custom_title_bar.dart';"
if target_import in g_c:
    g_c = g_c.replace(target_import, "import '../widgets/custom_title_bar.dart';\nimport 'game_player_screen.dart';")

target_launch = """  void _launchGame() {
    if (_gameInfo != null) {
      Navigator.push(
        context,
        MaterialPageRoute(
...
if target_launch in g_c:
    g_c = g_c.replace(target_launch, new_launch)
else:
    print("Could not find _launchGame")

with open('lib/screens/game_detail_screen.dart', 'w', encoding='utf-8') as f:
    f.write(g_c)

print("Updated game_detail_screen.dart")
