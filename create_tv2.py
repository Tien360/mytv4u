...
with codecs.open('lib/screens/tv_player_screen.dart', 'rb') as f:
    raw = f.read()
...
# 3. Add _initLocalServer before initState
...
<html><head><meta charset="utf-8"><title>TV DRM Player</title>
...
</head><body><video id="video" width="100%" height="100%" autoplay></video>
<script>
const manifestUri=\\'${_currentActualUrl}\\';
const ck=${_currentDrmKeys.isNotEmpty ? _currentDrmKeys : \\'{}\\'};
async function init(){const v=document.getElementById(\\'video\\');const p=new shaka.Player(v);
if(Object.keys(ck).length>0)p.configure({drm:{clearKeys:ck}});
try{await p.load(manifestUri)}catch(e){console.error(e)}}
...
'''
code = code.replace("  @override\r\n  void initState() {\r\n    super.initState();", init_server + "  @override\r\n  void initState() {\r\n    super.initState();\r\n    _initLocalServer();")
...
if old_play in code:
    ...
    print("ERROR: old_play NOT FOUND!")

# 5. Replace args URL for external player
...
if old_args in code:
    ...
    print("ERROR: old_args NOT FOUND!")

# 6. Replace webview loadUrl
