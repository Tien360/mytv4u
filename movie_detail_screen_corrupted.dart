...
class _MovieDetailScreenState extends State<MovieDetailScreen> {
  ...
  String _activeStandardSubTab = 'Vietsub'; // 'Vietsub' or 'Thuy?t Minh'
        ...
        PhimApi.fetchMovieDetailStream(
          ...
          (movie) {
            if (mounted) {
              ...
              // B?t d?u timer phát trailer sau khi có d? li?u d?u tiên
  ...
  Future<void> _fetchTmdbRating(Movie movie) async {
    try {
      ...
      // Luôn c? g?ng l?y backdrop ch?t lu?ng cao t? TMDB d? làm poster ngang
  ...
  void _categorizeServers(List<EpisodeServer> servers) {
    ...
    for (var server in servers) {
      ...
      } else if (name.contains('thuy?t minh') ||
          name.contains('l?ng ti?ng') ||
          name.contains('tm')) {
    ...
    // Ch? set default server n?u chua có server nào du?c ch?n
    // ho?c server dang ch?n không còn t?n t?i trong danh sách m?i
    ...
    if (!currentStillExists) {
      ...
      // C?p nh?t _currentServer sang object m?i nh?t (có th? episode list dã update)
  ...
  Future<void> _startInlineTrailer() async {
    ...
    final isTv =
        ...
        _movie!.currentEpisode.toLowerCase().contains('t?p');
      ...
      if (mounted) {
        setState(() {
          ...
          if (_autoPlayTrailerSetting) {
            ...
            _showInlineTrailer = false;
            _isTrailerPaused = false; // Ð? UI hi?n 'Phát Trailer' ban d?u
          }
          _trailerEnded = false;
          _isTrailerExpanded = false; // Luôn thu nh? khi m?i b?t d?u
        });
      }
    } else {
      if (mounted) {
...
''';
      ...
      onTap: () {
        setState(() {
          ...
          if (id == 'Vietsub' && _vietsubServers.isNotEmpty)
            _currentServer = _vietsubServers.first;
          else if (id == 'Thuy?t Minh' && _tmServers.isNotEmpty)
            _currentServer = _tmServers.first;
          else
            _currentServer = null;
        });
  ...
  Widget _buildEpisodesGrid() {
    ...
    // Embed servers (VidSrc, VidAPI) dùng slug S1E1 nhung không ph?i P2P,
    // chúng c?n du?c hi?n th? d?ng season nhung m? tr?c ti?p qua PlayerScreen
    final isEmbedServer = serverNameLower.contains('vidsrc') ||
        serverNameLower.contains('vidapi') ||
        serverNameLower.contains('embed');

    // Ch? kích ho?t season UI khi server là P2P ho?c Embed có slug S1E1
    ...
    if (_selectedSeason == null && seasons.isNotEmpty) {
      ...
      // Try to parse season from movie name (e.g., 'Ph?n 3', 'Season 3')
      final match = RegExp(r'(?:season|ph?n|part)\s*(\d+)', caseSensitive: false).firstMatch(_movie!.name);
      if (match != null) {
          ...
          Wrap(
            ...
            children: seasonsMap[_selectedSeason!]!.map((ep) {
              ...
              return GestureDetector(
                onTap: () async {
                  if (isP2p) {
                    // Ch? P2P/Torrent m?i c?n fetch streams riêng
                    _fetchP2pStreamsForEpisode(ep);
                  } else {
                    // Embed servers (VidSrc, VidAPI) và các server thu?ng
                    // d?u m? th?ng PlayerScreen v?i index dúng trong danh sách
                    _pauseTrailer();
                    FirebaseApi.saveContinueWatching(_movie!, ep.name);
  ...
  Widget _buildServerTab(
    ...
    if (isMotchillGroup) {
      ...
      displayName = displayName
          .replaceAll(
            RegExp(
              r'\s*-\s*(vietsub|thuy?t minh|l?ng ti?ng|tm|thuyet minh)',
              caseSensitive: false,
            ),
            '',
          )
        ...
        children: [
          ...
          Positioned.fill(
            child: BackdropFilter(
              ...
              child: Container(
                color: Colors.black.withOpacity(0.6), // Ph? den nh? lên màu g?c
              ),
            ),
          ),
          ...
          Positioned.fill(
            child: IgnorePointer(
              ignoring:
                  _isTrailerExpanded, // Khi m? r?ng trailer thì không ch?n tuong tác
              child: AnimatedOpacity(
                duration: const Duration(milliseconds: 400),
                opacity: _isTrailerExpanded ? 0.0 : 1.0,
                child: CustomScrollView(
                  slivers: [
                    // Kho?ng tr?ng d? l? ph?n Video bên trên
                    SliverToBoxAdapter(
                      child: GestureDetector(
                    ...
                    // Ph?n Content dè lên du?i c?a Video
                    SliverToBoxAdapter(
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          child: Column(
                            ...
                            children: [
                                          ...
                                          children: [
                                            ElevatedButton.icon(
                                              onPressed: () {
                                                ...
                                                UIUtils.showCustomSnackBar(
                                                  context,
                                                  '??? Ðã copy link Xem Phim: $shareUrl',
                                                );
                                              },
                                              icon: const Icon(
                                                Icons.share,
                              ...
                              if (_movie!.episodes.isEmpty) ...[
                                ...
                                // --- N?I DUNG THEO TAB CHÍNH ---
                                if (_activeMainTab == 'Promax') ...[
                                  // Hi?n th? các nút ch?n ngu?n Promax (Premium, P2P)
                                  Builder(
                                    builder: (context) {
                                      final promaxServers = [
                                        ..._premiumServers,
                                  ...
                                  // Sub-tabs: Thuy?t minh / Ph? d?
                                  Row(
                                    children: [
                                      _buildSubTab('Vietsub', 'Ph? d?'),
                                      const SizedBox(width: 12),
                                      _buildSubTab(
                                        'Thuy?t Minh',
                                        'Thuy?t minh / L?ng ti?ng',
                                      ),
                                    ],
                                  ),
                                  const SizedBox(height: 16),

                                  // Hi?n th? các ngu?n Standard d?a theo Sub-tab
                                  Builder(
                                    builder: (context) {
                                      final targetServers =
                                      ...
                                      if (targetServers.isEmpty)
                                        return Text(
                                          'Không có ngu?n nào trong m?c này.',
                                          style: TextStyle(
                                            color: Colors.white54,
                                          ),
                                        );
                              ...
                              const SizedBox(height: 100), // Ð?m du?i cùng
                            ],
                          ),
                        ),
                      ),
    ...
    return Row(
      children: [
        ...
        if (_isSubmittingRating)
          ...
          Text(
            '${_averageRating.toStringAsFixed(1)}/5 ($_totalRatings lu?t)',
            style: const TextStyle(
              color: Colors.amber,
              fontWeight: FontWeight.bold,
              fontSize: 16,
    ...
    return Column(
      ...
      children: [
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: _currentUser == null
                            ? L10n.t('login_to_comment') ?? 'Ðang nh?p d? bình lu?n...'
                            : L10n.t('write_your_comment') ?? 'Vi?t bình lu?n c?a b?n...',
                        hintStyle: TextStyle(
              ...
              ElevatedButton.icon(
                ...
                label: Text(
                  _currentUser == null ? L10n.t('login_and_send') ?? 'Ðang nh?p & G?i' : L10n.t('send_comment') ?? 'G?i bình lu?n',
                ),
                style: ElevatedButton.styleFrom(
        ...
        if (_isLoadingComments)
          ...
          Text(
            L10n.t('no_comments_yet') ?? 'Chua có bình lu?n nào.',
        ...
        if (!_isLoadingComments && _comments.length > _visibleComments)
          Padding(
            ...
            child: TextButton(
              ...
              child: Text(
                L10n.t('view_more_comments') ?? 'Xem thêm bình lu?n',
  ...
  String _formatTimeAgo(DateTime time) {
    ...
    if (diff.inDays > 365) return '${(diff.inDays / 365).floor()} nam tru?c';
    if (diff.inDays > 30) return '${(diff.inDays / 30).floor()} tháng tru?c';
    if (diff.inDays > 0) return '${diff.inDays} ngày tru?c';
    if (diff.inHours > 0) return '${diff.inHours} gi? tru?c';
    if (diff.inMinutes > 0) return '${diff.inMinutes} phút tru?c';
    return 'V?a xong';
  }
}
  ...
  void _openGallery() async {
    ...
    if (mounted) {
      ...
      if (images.isNotEmpty) {
        ...
        UIUtils.showCustomSnackBar(context, 'Không tìm th?y hình ?nh nào t? TMDB', isError: true);
      }
    }
  }
