...
# Let's replace the entire _showSleepTimerDialog method
    ...
    setState(() {
      ...
      if (minutes > 0) {
        _sleepTimer = Timer(Duration(minutes: minutes), () {
          if (mounted) {
            player.pause();
            Navigator.pop(context); // Thoát khỏi PlayerScreen
          }
        });
      }
    });
...
pattern = r"  void _showSleepTimerDialog\(\) \{[\s\S]*?\}\s*\}\s*\)\s*;\s*\n  \}"
content = re.sub(pattern, new_method, content)

with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated sleep timer dialog")
