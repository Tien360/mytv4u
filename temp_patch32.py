...
if "Future<void> _openYoutubeLogin" not in content:
    match = re.search(r'(\s*Widget _buildLoginCard\(\) \{)', content)
    if match:
        content = content[:match.start()] + func + content[match.start():]
        print("Re-injected _openYoutubeLogin with regex")
        with open('lib/screens/settings_screen.dart', 'w', encoding='utf-8') as f:
            f.write(content)
    else:
        print("Could not find _buildLoginCard with regex either")
