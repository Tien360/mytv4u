...
# Since old_yt1 is exact, I can just find its start and end
# We'll just replace the text by finding the index of `// YouTube` and then the index of `_buildLoginCard` and `_buildModernSourceCard`
...
if idx1 != -1:
    ...
    if end_idx1 != -1:
        ...
        print("Could not find end of user card")

# 2. Replace in _buildLoginCard
idx2 = content.find("          // YouTube", idx1 + len(new_yt1))
if idx2 != -1:
    end_idx2 = content.find("        ],\n      ),\n    );\n  }\n\n  Widget _buildModernSourceCard", idx2)
    if end_idx2 != -1:
        content = content[:idx2] + new_yt1 + "\n" + content[end_idx2:]
        print("Replaced YouTube row in login card!")
    else:
        print("Could not find end of login card")
