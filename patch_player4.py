...
# 2. Add methods to _PlayerScreenState
...
if "_isAudioOnly" not in content:
    if "  Widget _buildNextEpisodeOverlay() {" in content:
        content = content.replace("  Widget _buildNextEpisodeOverlay() {", audio_code + "  Widget _buildNextEpisodeOverlay() {")
    else:
        print("ERROR: Anchor _buildNextEpisodeOverlay not found!")
        sys.exit(1)

# 3. Replace ONLY the Video widget in the main stack
video_target = """                  else
                    Center(
                      child: Video(
...
if "if (_isAudioOnly)" not in content:
    if video_target in content:
        content = content.replace(video_target, video_replacement, 1)
    else:
        print("ERROR: Video target not found!")
        sys.exit(1)

with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("SUCCESS: Patched player_screen.dart")
