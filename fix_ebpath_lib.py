...
def patch(filepath):
    ...
    old_block = r"final appDataDir = await getApplicationSupportDirectory\(\);\s*final ebPath = p\.join\(appDataDir\.path, 'youtube_webview_profile', 'EBWebView'\);"
    ...
    if match:
        ...
        print("Could not find old block in library_screen")
        
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

patch('lib/screens/library_screen.dart')
print("Patched library_screen.dart with smart cookie path")
